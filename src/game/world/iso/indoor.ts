...
const stool: Draw = (ctx) => {
  ...
  boxAt(ctx.painter, ctx.at, { w: 0.16, d: 0.16, h: 17 }, iron);
  boxAt(ctx.painter, shift(ctx.at, 0, 0, 17), { w: 0.5, d: 0.5, h: 4 }, skin(ctx, 0x8f5a4a));
};

/** Стойка: длинный прилавок со столешницей и подсветкой цоколя. */
const counter: Draw = (ctx) => {
  shade(ctx, 2.2, 1);
  boxAt(ctx.painter, ctx.at, { w: 2.1, d: 0.9, h: 23 }, skin(ctx, 0x5a4a6a));
  boxAt(ctx.painter, shift(ctx.at, 0, 0, 23), { w: 2.35, d: 1.05, h: 4 }, skin(ctx, 0xd8c8a8));
...
const speaker: Draw = (ctx) => {
  shade(ctx, 0.7, 0.7);
  boxAt(ctx.painter, ctx.at, { w: 0.7, d: 0.7, h: 57 }, skin(ctx, 0x232733));
  for (const [top, height] of [[54, 13], [36, 13], [18, 8]] as const) {
...
const shelf: Draw = (ctx) => {
  ...
  boxAt(ctx.painter, ctx.at, { w: 1, d: 0.3, h: 39 }, wood);
  for (let i = 0; i < 3; i += 1) {
    const lift = 8 + i * 12;
...
const seat: Draw = (ctx) => {
  ...
  boxAt(ctx.painter, ctx.at, { w: 1.3, d: 0.95, h: 16 }, {
    top: scale(body, 1.35),
    left: scale(body, 0.6),
    right: scale(body, 0.85),
    outline: 0x0b0a12,
  });
  panel(ctx.painter, ctx.at, 'x', { span: 1.3, across: 0.475, top: 16, height: 2 }, glow, 0.85);
  panel(ctx.painter, ctx.at, 'y', { span: 0.95, across: 0.65, top: 16, height: 2 }, glow, 0.6);
...
const screen: Draw = (ctx) => {
  ...
  const span = 2;
  panel(ctx.painter, ctx.at, axis, { span: span + 0.12, across: 0, top: 72, height: 64 }, 0x0d0b16);
  ...
  panel(ctx.painter, ctx.at, axis, { span: span + 0.16, across: 0, top: 74, height: 2 }, 0x3a3556);
};
...
const windowPane: Draw = (ctx) => {
  ...
  panel(ctx.painter, ctx.at, axis, { span: 1.7, across: 0, top: 67, height: 57 }, frame);
  panel(ctx.painter, ctx.at, axis, { span: 1.5, across: 0, top: 63, height: 49 }, glass);
  panel(ctx.painter, ctx.at, axis, { span: 1.5, across: 0, top: 63, height: 16 }, scale(glass, 1.12));
  // Переплёт: горбылёк вдоль стены и стойка поперёк неё.
  panel(ctx.painter, ctx.at, axis, { span: 1.5, across: 0, top: 40, height: 2 }, frame);
  panel(ctx.painter, ctx.at, axis, { span: 0.07, across: 0, top: 63, height: 49 }, frame);
...
const poster: Draw = (ctx) => {
  ...
  panel(ctx.painter, ctx.at, axis, { span: 1.15, across: 0, top: 57, height: 36 }, scale(0x2a2430, ctx.ambience.light));
  panel(ctx.painter, ctx.at, axis, { span: 1, across: 0, top: 56, height: 34 }, art);
  panel(ctx.painter, ctx.at, axis, { span: 0.8, across: 0, top: 52, height: 13 }, 0x000000, 0.28);
  panel(ctx.painter, ctx.at, axis, { span: 0.8, across: 0, top: 36, height: 2 }, 0xffffff, 0.45);
  panel(ctx.painter, ctx.at, axis, { span: 0.5, across: 0, top: 31, height: 2 }, 0xffffff, 0.3);
};
