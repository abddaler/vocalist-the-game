...
const trafficLight: Draw = (ctx) => {
  shade(ctx, 0.3, 0.3);
  const axis = ctx.facing;
  boxAt(ctx.painter, ctx.at, { w: 0.14, d: 0.14, h: 34 }, skin(ctx, 0x3a3f4a));
  boxAt(ctx.painter, shift(ctx.at, 0, 0, 32), { w: 0.4, d: 0.4, h: 14 }, skin(ctx, 0x22242c, 0.3));
  ...
  eyes.forEach(([color, alpha], i) => {
    panel(ctx.painter, ctx.at, axis, { span: 0.22, across: 0.2, top: 43 - i * 4, height: 3 }, color, alpha);
  });
};
...
const surfboard: Draw = (ctx) => {
  ...
  panel(ctx.painter, ctx.at, axis, { span: 0.5, top: 26, height: 20 }, deck);
  panel(ctx.painter, ctx.at, axis, { span: 0.36, top: 32, height: 6 }, deck);
  panel(ctx.painter, ctx.at, axis, { span: 0.36, top: 6, height: 6 }, deck);
  panel(ctx.painter, ctx.at, axis, { span: 0.08, top: 30, height: 26 }, 0xffffff, 0.45);
  // Кант с теневой стороны: он и даёт доске толщину.
  panel(
    ctx.painter,
    ctx.at,
    axis === 'x' ? 'y' : 'x',
    { span: 0.12, top: 30, height: 28 },
    scale(deck, 0.62),
  );
};
